...
import { CoreViewAdapter } from '@/components/Editor/adapters/BaseViewAdapter.optimized'
...
import { AdapterState } from '@/components/Editor/types/OptimizedViewAdapter'
import { useTheme } from '@/hooks/useTheme'
import { Button } from '@/components/ui/button'
import { EditorCoreCommands, EditorCoreState } from '../types/EditorCore'
...
export interface EditorCoreProps {
    ...
    /** 状态变化回调 */
    onStateChange?: (state: EditorCoreState) => void
    /** 错误处理回调 */
 ...
 * 优化后的编辑器核心组件
 */
const EditorCore = forwardRef<EditorCoreCommands, EditorCoreProps>(
    function EditorCore(props: EditorCoreProps, ref: React.Ref<EditorCoreCommands>) {
        const {
            className = '',
            style = {},
            initialAST,
        ...
        // === 状态管理 ===
        const [editorState, setEditorState] = useState<EditorCoreState>(EditorCoreState.UNINITIALIZED)
        ...
        // === 状态更新函数 ===
        const updateEditorState = useCallback((newState: EditorCoreState) => {
        ...
        const handleError = useCallback((error: Error, context: string) => {
            ...
            setError(error)
            updateEditorState(EditorCoreState.ERROR)
        ...
        const createAdapterInstance = useCallback(async () => {
            ...
            updateEditorState(EditorCoreState.INITIALIZING)
            ...
            try {
                ...
                adapter.setHooks({
                    ...
                    created: async () => {
                        updateEditorState(EditorCoreState.READY)
                    },
                    beforeDestroy: async () => {
                        updateEditorState(EditorCoreState.DESTROYED)
                    }
                })
        ...
        // === 命令接口实现 ===
        const commands: EditorCoreCommands = useMemo(() => ({
            getState: () => editorState,
            isReady: () => editorState === EditorCoreState.READY,
        ...
        // === 错误边界渲染 ===
        if (error && editorState === EditorCoreState.ERROR) {
            return (
        ...
        return (
            ...
            >
                {/* 加载状态 */}
                {editorState === EditorCoreState.INITIALIZING && (
                ...
                <div className="editor-viewport w-full h-full">
                    {adapterRef.current && editorState === EditorCoreState.READY && (
